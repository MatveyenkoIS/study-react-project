import styles from "./CartedCard.module.css";

function CartedCard({ cartedProduct, onOrder, onRemove }) {
  return (
    <div className={styles.card}>
      <div className={styles.cardHeader}>
        <h3 className={styles.title}>{cartedProduct.title}</h3>
        <span className={styles.icons}>
          {cartedProduct.rating >= 4 && "🔥"}
          {cartedProduct.isPremium && "💎"}
        </span>
      </div>
      <div className={styles.cardContent}>
        <span className={styles.price}>{cartedProduct.price}$</span>
        <div className={styles.buttonPad}>
          <button
            className={styles.button}
            onClick={() => onOrder(cartedProduct.title)}
          >
            Order
          </button>
          <button
            className={`${styles.button} ${styles.removeFromCart}`}
            onClick={() => onRemove(cartedProduct.id)}
          >
            <svg
